...
central_supabase_url = 'https://central-supabase-url'
central_supabase_key = 'central-supabase-key'
central_supabase = create_client(central_supabase_url, central_supabase_key)

# Enterprise configurations
enterprise_configs = {
    'Bloomberg': {
        'url': 'http://localhost:54322',  # Replace with actual URLs
        'key': 'your-bloomberg-supabase-key'
    },
    'Global Financial Corp': {
        'url': 'http://localhost:54323',
        'key': 'your-gfc-supabase-key'
    },
    # Add other enterprises as needed
}

# Function to get the Supabase client for a specific enterprise
def get_enterprise_supabase(enterprise_name):
    config = enterprise_configs.get(enterprise_name)
    if not config:
        raise ValueError(f"No Supabase configuration found for enterprise {enterprise_name}")
...
def sync_connections_table(local_supabase, connection):
    # Check if the connection exists in the local 'connections' table
    local_connection = local_supabase.table("connections").select("*").eq("central_connection_id", connection['id']).execute()
    if not local_connection.data:
        # Insert the connection into the local 'connections' table
        data = {
            'central_connection_id': connection['id'],
            'source_division_id': connection['source_division_id'],
            'target_division_id': connection['target_division_id'],
            'daily_messages_count': connection['daily_messages_count']
        }
        local_supabase.table("connections").insert(data).execute()
    else:
        # Optionally update the local connection if needed
        pass
...
#     # Query for the connection in both possible directions in the central database
#     query1 = central_supabase.table("connections").select("*").eq("source_division_id", sender_division_id).eq("target_division_id", receiver_division_id).execute()
#     query2 = central_supabase.table("connections").select("*").eq("source_division_id", receiver_division_id).eq("target_division_id", sender_division_id).execute()
...
# updated
CENTRAL_API_BASE_URL = 'http://central-database-url.com/api'  # Replace with actual URL

def find_connection(sender_division_id, receiver_division_id):
    # Query the central database via API
    params = {
    ...
    if response.status_code == 200:
        ...
        print(f"Error querying central database: {response.status_code}")
        return None
...
def insert_data_policy(sender_division_id, receiver_division_id, confidentiality, data_type, explanation):
    ...
    # Insert data into the division's own 'data_policies' table
    data = {
        "connection_id": connection_id,
        "confidentiality": confidentiality,
        "data_type": data_type,
    ...
    result = supabase.table("data_policies").insert(data).execute()
    return result.data


def insert_data_policy(sender_division_id, receiver_division_id, confidentiality, data_type, explanation):
    # Find the connection in the central database
    connection = find_connection(sender_division_id, receiver_division_id)
    if not connection:
        return "No valid connection found between the specified divisions."

    # Get sender's company information from the central database
    company_info = central_supabase.table("divisions").select("company_id").eq("id", sender_division_id).execute()
    if not company_info.data:
        return "Sending division not found."
    company_id = company_info.data[0]['company_id']

    # Get company name to connect to the correct Supabase instance
    company = central_supabase.table("companies").select("name").eq("id", company_id).execute()
    if not company.data:
        return "Company not found."
    company_name = company.data[0]['name']
    sender_supabase = get_enterprise_supabase(company_name)

    # Synchronize the local connections table
    sync_connections_table(sender_supabase, connection)

    # Get the local connection ID
    local_connection = sender_supabase.table("connections").select("local_connection_id").eq("central_connection_id", connection['id']).execute()
    if not local_connection.data:
        return "Connection not found in sender's local connections table."
    local_connection_id = local_connection.data[0]['local_connection_id']

    # Insert data into the sender's 'data_policies' table
    data = {
    ...
    result = sender_supabase.table("data_policies").insert(data).execute()
    return result.data
...
def log_message(sender_division_id, receiver_division_id, connection, message, thread_id="new thread", print_statements=False):
    ...
    # Log message in sender's local database
    ...
    if thread_id == "new thread":
        # Create a new thread in the sender's local database
        thread_entry = {
            "connection_id": connection['id'],
            "last_message_timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "source_division_cost": 0.0001 if sender_division_id == connection['source_division_id'] else 0,
        ...
        thread_result = supabase.table("collab_threads").insert(thread_entry).execute()
        thread_id = thread_result.data[0]['thread_id']
        thread_msg_ordering = 1
    else:
        # Update existing thread in sender's local database
        thread = supabase.table("collab_threads").select("*").eq("thread_id", thread_id).execute().data[0]
        update_data = {
            "messages_count": thread['messages_count'] + 1,
            "last_message_timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "source_division_cost": thread['source_division_cost'] + (0.0001 if sender_division_id == connection['source_division_id'] else 0),
        ...
        supabase.table("collab_threads").update(update_data).eq("thread_id", thread_id).execute()
        thread_msg_ordering = thread['messages_count'] + 1
